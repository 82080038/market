...
## Status: RENOVATION IN PROGRESS — New app at `/home/petrick/projects/quant/`
## Date: 2026-08-19 (updated 2026-08-20)

> **Note:** Berdasarkan audit lengkap 7-layer pipeline ini, diputuskan untuk membangun aplikasi tandingan di `/home/petrick/projects/quant/` dengan database PostgreSQL baru (`quant`) dan schema point-in-time native. Raw data sudah di-transfer dari `market` DB. Signal engines, backtest, portfolio, dan execution modules yang KEEP sudah di-copy. Aplikasi `market` tetap sebagai reference dan source of truth untuk data historis. Lihat `docs/ENGINE_AUDIT_MATRIX.md` di project `quant` untuk versi terbaru.
