...
- Migrations: 0001-0036 (alembic head = 0036). Lihat `alembic/versions/`. Migration 0022: normalisasi database (drop `broker`/`broker_bursa`, merge `market_registry`→`exchanges`, merge `instrument_master`→`instruments`, drop prediction columns dari `stock_personality`, add FK constraints). Lihat `docs/DATABASE_NORMALIZATION_0022.md`. Migration 0023: merge `market_calendar`→`exchange_holidays` (compatibility view dibuat). Migration 0024: `instrument_behavior_profiles` (28 kolom profile per instrumen). Migration 0025: `cross_market_coefficients` (Granger causality + asymmetric up/down + regime). Migration 0026: `user_trading_profiles` + `trading_style_recommendations` + `style_recommendation_reasons`. Migration 0027: fetch metadata columns di `instruments`. Migration 0028: data source + delisting/merger metadata di `instruments`. Migration 0029: consolidate data source metadata (fetch routing). Migration 0030: `market_influence_kb` (central influence mapping table, 4,038 records). Migration 0031: `scheduler_state` upgrade (next_run_at, stale detection, data dependencies). Migration 0032: `signal_weights` (DB-backed dynamic weight configuration). Migration 0033: `recompute_dependencies` (dependency graph tables). Migration 0034: recompute stats columns. Migration 0035: `recompute_predictions` (pre-computed duration/row estimates). Migration 0036: tracking columns di `recompute_dependencies` + `data_watermark`.
