...
- Basis pengetahuan berada di `<PROJECT_DIR>/pustaka/` — 104 dokumen Markdown bernomor `00-README.md` sampai `103-*.md`. Lihat §7 untuk padanan path per-OS.
...
- Market influence knowledge base: `pustaka/103-market-influence-knowledge-base.md` (tabel `market_influence_kb`, 4,038 records, 7 influence types)
...
- Market session status: `src/market/utils/market_session.py` (`MarketSessionManager`, 10 bursa, DST-aware, `should_run_pipeline`)
- Instrument behavior profiler: `src/market/analysis/instrument_profiler.py` (`InstrumentBehaviorProfiler`, `profile_all_instruments`, `detect_regime_change`, tabel `instrument_behavior_profiles`)
- Cross-market coefficients: `src/market/analysis/cross_market_coefficients.py` (`CrossMarketCoefficientEngine`, Granger + asymmetric up/down, tabel `cross_market_coefficients`)
- Trading style advisor: `src/market/advisory/trading_style_advisor.py` (`TradingStyleAdvisor`, `recommend_style`, tabel `user_trading_profiles`)
- Enhanced signal generator: `src/market/analysis/enhanced_signal_generator.py` (`EnhancedSignalGenerator`, wrapper profile + cross-market + suitability filter)
- Capital-aware position sizer: `src/market/risk/capital_aware_sizer.py` (`CapitalAwarePositionSizer`, quarter-Kelly + liquidity + risk caps)
- Recommendation engine: `src/market/analysis/recommendation_engine.py` (`RecommendationEngine`, `generate_report`, output Bahasa Indonesia `to_text_summary`/`to_json`)
...
- Modul analisis baru: `src/market/analysis/` — `astronacci.py`, `macro_correlation.py`, `strategy_selector.py`, `pairs_trading.py`, `volume_features.py`, `policy_event_scorer.py`, `sector_rotation.py`, `signal_enhancer.py`, `meta_labeling.py`, `news_sentiment.py`, `cross_market_timezone.py`, `execution_analyzer.py`, `alpha_signals.py` (4 alpha signal engines: mean_reversion, reversal, ewma_momentum, regime_switch), `instrument_profiler.py` (InstrumentBehaviorProfiler + tabel `instrument_behavior_profiles`), `cross_market_coefficients.py` (CrossMarketCoefficientEngine + tabel `cross_market_coefficients`), `enhanced_signal_generator.py` (wrapper integrasi profile + cross-market), `recommendation_engine.py` (RecommendationEngine output final Bahasa Indonesia), `market_influence_kb.py` (MarketInfluenceKB + tabel `market_influence_kb`), `holiday_effect.py` (HolidayEffectAnalyzer), `recompute_graph.py` (RecomputeDependencyGraph), `recompute_estimator.py` (RecomputeEstimator), `recompute_analyzer.py` (RecomputeAnalyzer), `weight_registry.py` (WeightRegistry + tabel `signal_weights`)
- Modul advisory baru: `src/market/advisory/` — `trading_style_advisor.py` (TradingStyleAdvisor + tabel `user_trading_profiles`/`trading_style_recommendations`/`style_recommendation_reasons`)
- Modul utils baru: `src/market/utils/` — `market_session.py` (MarketSessionManager untuk 10 bursa utama, DST-aware, integrasi cron)
- Modul risk baru: `src/market/risk/` — `capital_aware_sizer.py` (CapitalAwarePositionSizer, quarter-Kelly + liquidity + risk caps)
- Modul data baru: `src/market/data/` — `macro_data_fetcher.py`, `satellite_fetcher.py`, `refresh_stale.py`, `timestamp_validation.py`, `fetch_registry.py` (FetchRegistry — DB-as-source-of-truth untuk data fetching), `idx_adapter.py` (IDX Official Adapter — idx.co.id API via cloudscraper)
- API routes baru: `src/market/api/routes_notifications.py`, `src/market/api/routes_recompute.py`
- API cache: `src/market/api/cache.py` (TTL cache untuk expensive API endpoints)
- Migrations: 0001-0039 (alembic head = 0039). Lihat `alembic/versions/`. Migration 0022: normalisasi database (drop `broker`/`broker_bursa`, merge `market_registry`→`exchanges`, merge `instrument_master`→`instruments`, drop prediction columns dari `stock_personality`, add FK constraints). Lihat `docs/DATABASE_NORMALIZATION_0022.md`. Migration 0023: merge `market_calendar`→`exchange_holidays` (compatibility view dibuat). Migration 0024: `instrument_behavior_profiles` (28 kolom profile per instrumen). Migration 0025: `cross_market_coefficients` (Granger causality + asymmetric up/down + regime). Migration 0026: `user_trading_profiles` + `trading_style_recommendations` + `style_recommendation_reasons`. Migration 0027: fetch metadata columns di `instruments`. Migration 0028: data source + delisting/merger metadata di `instruments`. Migration 0029: consolidate data source metadata (fetch routing). Migration 0030: `market_influence_kb` (central influence mapping table, 4,038 records). Migration 0031: `scheduler_state` upgrade (next_run_at, stale detection, data dependencies). Migration 0032: `signal_weights` (DB-backed dynamic weight configuration). Migration 0033: `recompute_dependencies` (dependency graph tables). Migration 0034: recompute stats columns. Migration 0035: `recompute_predictions` (pre-computed duration/row estimates). Migration 0036: tracking columns di `recompute_dependencies` + `data_watermark`. Migration 0037: `corporate_actions` table add `corporate_calendar` source columns. Migration 0038: `broker_daily_summary` + `alert_log` tables. Migration 0039: `corporate_calendar` table (IDX corporate events: RUPS, buyback, dividend, 251 rows).
...
- Corporate calendar ML integration: `PolicyEventScorer._load_corporate_calendar()` loads IDX corporate events (RUPS, buyback, dividend, 251 rows) from `corporate_calendar` table → maps to EventType/EventDirection/base_impact → `SignalEnhancer._compute_event_signal()` produces directional bias. `SignalEnhancer._compute_broker_momentum_signal()` reads `broker_daily_summary` (market-wide broker activity momentum: value trend + frequency trend + top-10 concentration, weight 0.10). `MultiFactorFeaturePipeline.build(broker_liquidity=...)` adds 6 liquidity regime features (`liq_total_value_log`, `liq_total_volume_log`, `liq_freq_log`, `liq_n_brokers`, `liq_top10_conc`, `liq_val_mom_3d`). `load_broker_liquidity()` helper di `multi_factor.py`. Recompute dependencies registered: `corporate_calendar`, `news_sentiment`, `broker_daily_summary` → `recompute_scores` (optional).
