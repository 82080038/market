...
| 101 | `101-global-idx-advanced-models.md` | **Global-IDX Advanced Models** | Model lanjutan untuk integrasi global→IDX: DCC-GARCH (dynamic conditional correlation), Diebold-Yilmaz spillover index, foreign flow momentum (net buy/sell trend), overnight IDX signal (US T-1 + Asian T-0). Implementasi di `src/market/analysis/cross_market_timezone.py`, `src/market/analysis/macro_correlation.py`. Terdaftar di engine registry ablation framework. |
| 102 | `102-sector-global-link-engine.md` | **Sector-Global Link Engine** | Engine sektor-specific global driver dengan timezone lag — mapping sektor IDX ke global driver (mis. IDXENERGY→crude oil, IDXFINANCE→US 10Y yield), 60-day rolling correlation, z-score signal, weight berdasarkan kekuatan korelasi. Implementasi di engine registry ablation framework sebagai `sector_global_link`. |
| 103 | `103-market-influence-knowledge-base.md` | **Market Influence Knowledge Base** | Basis pengetahuan terpusat untuk mapping influence — tabel `market_influence_kb` (4,038 records) mengonsolidasi 5 sumber: sector-global link (pustaka/102), commodity sensitivity, Granger causality, cross-market coefficients, macro policy. Module `src/market/analysis/market_influence_kb.py` dengan `compute_influence_signal()` untuk sinyal agregat [-1,1]. Migration 0030. |
...
85. Dokumen 101 berisi Global-IDX Advanced Models: 4 model advanced untuk hubungan pasar global dengan IDX — DCC-GARCH (dynamic conditional correlation Engle 2002, simplified GARCH(1,1) + DCC tanpa full MLE), Diebold-Yilmaz spillover index (VAR-based variance decomposition, walk-forward re-estimation 60d), Foreign Flow prediction (linear scoring: BI-Fed rate differential, VIX, USD/IDR), Overnight Global → IDX Opening (US T-1 + Asian T-0 composite, timezone-aware), anti look-ahead bias compliance, implementasi sebagai 4 ablation engine terdaftar di registry (total 28), hasil ablation: dcc_garch positive ΔAlpha (+0.0853)
86. Dokumen 102 berisi Sector-Global Link Engine: engine yang memetakan setiap sektor IDX ke global market driver yang relevan dengan timezone-aware lag — mapping 11 sektor IDX ke global tickers (Energy→CL=F, Financials→^TNX, Basic Materials→GC=F, Tech→^IXIC, Consumer→IDR=X, dll), subsector override (Gold→GC=F, Banks→^TNX, Telecom→^IXIC), timezone lag (T-0 Asian, T-1 US/Europe), signal direction per driver, multi-driver consensus, threshold 0.5%, anti look-ahead compliance, implementasi sebagai ablation engine sector_global_link (total 29 engine), hasil ablation: ΔSharpe=-1.4659 (perlu refinement threshold dinamis & rolling correlation direction)
